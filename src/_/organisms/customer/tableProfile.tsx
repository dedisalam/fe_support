import React from 'react';
import { ButtonGroup, Table } from 'react-bootstrap';
import ButtonDelete from '_/atoms/buttonDelete';
import ButtonEdit from '_/atoms/buttonEdit';
import { isEmptyArray } from '_/utils';

type TableProfileProps = {
  data: {
    id: number;
    name: string;
    address: string;
    telp: string;
    hp: string;
    fax: string;
  }[];
  onClickDelete: (id: number) => void;
  onClickEdit?: (id: number) => string | undefined;
  isMutate: boolean;
};

function TableProfile(props: TableProfileProps) {
  const {
    data, onClickEdit, onClickDelete, isMutate,
  } = props;

  if (isEmptyArray(data)) {
    return <div>No data</div>;
  }

  return (
    <Table striped responsive hover>
      <thead>
        <tr>
          <th>No</th>
          <th>Name</th>
          <th>Address</th>
          <th>Phone</th>
          <th>Handphone</th>
          <th>Fax</th>
          <th>Action</th>
        </tr>
      </thead>
      <tbody>
        {data.map((item, index) => (
          <tr key={item.id}>
            <td>{index + 1}</td>
            <td>{item.name}</td>
            <td>{item.address}</td>
            <td>{item.telp}</td>
            <td>{item.hp}</td>
            <td>{item.fax}</td>
            <td>
              <ButtonGroup aria-label="Basic example">
                {onClickEdit !== undefined && (
                <ButtonEdit
                  disabled={isMutate}
                  onClick={onClickEdit(item.id)}
                />
                )}
                <ButtonDelete disabled={isMutate} onClick={() => { onClickDelete(item); }} />
              </ButtonGroup>
            </td>
          </tr>
        ))}
      </tbody>
    </Table>
  );
}

TableProfile.defaultProps = {
  onClickEdit: undefined,
};

export default TableProfile;
